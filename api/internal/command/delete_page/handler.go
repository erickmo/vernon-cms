...
func (c Command) CommandName() string { return "DeletePage" }
...
func NewHandler(repo page.WriteRepository, eventBus eventbus.EventBus) *Handler {
	return &Handler{
		repo:     repo,
		eventBus: eventBus,
		tracer:   otel.Tracer("command.delete_page"),
	}
}
...
func (h *Handler) Handle(ctx context.Context, cmd commandbus.Command) error {
	...
	ctx, span := h.tracer.Start(ctx, "DeletePage.Handle")
	defer span.End()
	...
	return h.eventBus.Publish(ctx, page.PageDeleted{
		PageID: c.ID,
		Time:   time.Now(),
	})
}
