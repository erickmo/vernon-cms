...
func (q Query) QueryName() string { return "GetDataRecord" }
...
func NewHandler(repo data.DataReadRepository) *Handler {
	return &Handler{repo: repo, tracer: otel.Tracer("query.get_data_record")}
}

func (h *Handler) Handle(ctx context.Context, q querybus.Query) (interface{}, error) {
	query := q.(Query)
	_, span := h.tracer.Start(ctx, "GetDataRecord.Handle")
	defer span.End()

	return h.repo.FindRecordByID(query.ID, query.SiteID)
}
