...
func (q Query) QueryName() string { return "GetContent" }
...
func NewHandler(repo content.ReadRepository, cache *redis.Client, metrics *telemetry.Metrics, ttl time.Duration) *Handler {
	return &Handler{
		...
		cache:   cache,
		metrics: metrics,
		tracer:  otel.Tracer("query.get_content"),
		ttl:     ttl,
	}
}
...
func (h *Handler) Handle(ctx context.Context, q querybus.Query) (interface{}, error) {
	...
	ctx, span := h.tracer.Start(ctx, "GetContent.Handle")
	defer span.End()

	cacheKey := fmt.Sprintf("content:%s:%s", query.SiteID.String(), query.ID.String())
	...
	ct, err := h.repo.FindByID(query.ID, query.SiteID)
	if err != nil {
		return nil, err
	}
...
func toReadModel(ct *content.Content) *ReadModel {
	return &ReadModel{
		ID:          ct.ID,
		SiteID:      ct.SiteID,
		Title:       ct.Title,
